...
library(DT);library(dplyr);library(ggplot2);library(ggthemes);library(mclust);
library(ggforce);library(shinyjs);library(plotly)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = "school", "School", choices = names, 
                  selected = "Montana State University", multiple = FALSE,
                  selectize = TRUE), 
      useShinyjs(),
      hr(),
...
cc18Ps <-
  cc18[,c("NAME","BASIC2010","BASIC2015","BASIC2018","FACNUM","HUM_RSD","OTHER_RSD",
          "SOCSC_RSD","STEM_RSD","PDNFRSTAFF","S.ER.D","NONS.ER.D")]
cc18Ps.rm <- filter(cc18Ps, BASIC2018 %in% c(15,16)) #removes those pesky R3 schools
cc18Ps <- na.omit(as_tibble(cbind(cc18Ps.rm[,c(1:4)],
                                  lapply(cc18Ps.rm[,5:12],replace_0)))) #replaces with 0's
## do the same for 2021:
cc21Ps <- CC2021[,c("name","basic2021","facnum","hum_rsd","oth_rsd","socsc_rsd",
                    "stem_rsd","pdnfrstaff","serd","nonserd")]
cc21Ps.rm <- filter(cc21Ps, basic2021 %in% c(15,16)) 
cc21Ps <- na.omit(as_tibble(cbind(cc21Ps.rm[,c(1:2)],lapply(cc21Ps.rm[,3:10],
                                                            replace_0)))) #replaces with 0's
dim(cc21Ps)  
...
server <- function(input, output,session) {
  ...
  output$classPlot21 <- renderPlotly({
    ...
    scores21$Symbols <- rep(10, nrow(scores21))
  ...
  output$table.out21 <- renderTable({
    ...
    a <- which(as.character(cc21Ps$name) == as.character(inst_name))
  ...
  output$table.out <- renderTable({
    ...
    a <- which(as.character(cc18Ps$NAME) == as.character(inst_name))
